...
export function useServerFilesManager(options: UseServerFilesManagerOptions) {
  ...
  const renameModal = reactive({ open: false, file: null as ServerFileListItem | null, value: '', loading: false })
  const chmodModal = reactive({ open: false, file: null as ServerFileListItem | null, value: '755', loading: false })
  const pullModal = reactive({ open: false, url: '', loading: false })
  const deleteModal = reactive({ open: false, file: null as ServerFileListItem | null, loading: false })
  ...
  const currentEntries = computed<ServerFileListItem[]>(() => {
    const files = directoryEntries.value.map((entry): ServerFileListItem => ({
      name: entry.name,
  ...
  const allSelected = computed(() => hasSelection.value && selectedCount.value === currentEntries.value.length && currentEntries.value.length > 0)
  ...
  function toggleSelectAllEntries(value: boolean) {
    if (value)
      selectedPaths.value = currentEntries.value.map(entry => entry.path)
    else
      clearSelection()
  }
  ...
  async function fetchDirectory(directory = currentDirectory.value, { force = false } = {}) {
    if (!force && lastRequestedDirectory.value === directory)
      return
    ...
    try {
      const response = await requestFetch<{ data: { entries: ServerFileEntry[] } }>(`${clientApiBase.value}/files/list`, {
        method: 'GET',
        query: { directory },
      })
  ...
  async function submitBulkMove() {
    ...
    try {
      const files = items.map(item => ({ from: item.path, to: joinDirectoryPath(destination, item.name) }))
      await requestFetch(`${clientApiBase.value}/files/rename`, {
        method: 'POST',
        body: {
          root: '/',
