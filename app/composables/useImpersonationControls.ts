...
export function useImpersonationControls(options?: UseImpersonationControlsOptions) {
  ...
  async function stopImpersonating(override?: StopImpersonatingOptions) {
    if (stopImpersonationLoading.value)
      return
    ...
    try {
      ...
      const redirectTarget = 'redirectTo' in (override ?? {})
        ? override?.redirectTo
      ...
      const shouldNotify = override?.notifications ?? options?.notifications ?? true
      if (shouldNotify) {
        const description = error instanceof Error ? error.message : t('layout.impersonationStopFailed')
        toast.add({
