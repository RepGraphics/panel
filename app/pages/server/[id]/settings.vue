...
<template>
  <UPage>
          ...
          <template v-else>
            <UCard>
              ...
              <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
                    {{ t('server.details.identifier') }}
                  </p>
                  <div class="mt-2 flex items-center justify-between gap-2">
                    <p class="font-mono text-sm text-foreground">{{ server?.identifier }}</p>
                    <ServerCopyButton
                      v-if="server?.identifier"
                      :text="server.identifier"
                      :label="t('server.details.identifier')"
                    />
                  </div>
                </div>
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
                    {{ t('server.details.uuid') }}
                  </p>
                  <div class="mt-2 flex items-center justify-between gap-2">
                ...
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  ...
                  <div class="mt-2 flex items-center gap-2">
                    <UBadge v-if="server?.suspended" color="error" size="sm">{{
                      t('common.suspended')
                    }}</UBadge>
            ...
            <UCard>
              ...
              <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
                    {{ t('server.details.cpu') }}
                  </p>
                  <p class="mt-2 text-lg font-semibold text-foreground">
                    {{ formatCpu(limits.cpu) }}
                  </p>
                </div>
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
                    {{ t('server.details.memory') }}
                  </p>
                  <p class="mt-2 text-lg font-semibold text-foreground">
                ...
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
                    {{ t('server.details.swap') }}
                  </p>
                  <p class="mt-2 text-lg font-semibold text-foreground">
                    {{ formatBytes(limits.swap) }}
                  </p>
                </div>
                <div class="rounded-md border border-default bg-muted/30 px-4 py-3">
                  <p class="text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <UCard v-if="server?.suspended">
              <template #header>
                <div class="flex items-center justify-between">
                  ...
                  <UButton icon="i-lucide-unlock" size="sm" color="warning" variant="soft">{{
                    t('common.suspended')
                  }}</UButton>
                </div>
              </template>
    ...
    >
      ...
      <template #footer>
        <UButton variant="ghost" :disabled="reinstalling" @click="showReinstallModal = false">
          {{ t('common.cancel') }}
        </UButton>
        <UButton
          icon="i-lucide-refresh-cw"
    ...
    >
      ...
      <template #footer>
        <UButton variant="ghost" :disabled="renaming" @click="showRenameModal = false">
          {{ t('common.cancel') }}
        </UButton>
        <UButton
          icon="i-lucide-check"
