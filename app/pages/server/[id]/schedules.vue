...
const cronPresets = computed(() => [
  { label: t('server.schedules.cronPresets.everyMinute'), value: { minute: '*', hour: '*', dayOfMonth: '*', month: '*', dayOfWeek: '*' } },
  { label: t('server.schedules.cronPresets.every5Minutes'), value: { minute: '*/5', hour: '*', dayOfMonth: '*', month: '*', dayOfWeek: '*' } },
  { label: t('server.schedules.cronPresets.everyHour'), value: { minute: '0', hour: '*', dayOfMonth: '*', month: '*', dayOfWeek: '*' } },
  { label: t('server.schedules.cronPresets.dailyAtMidnight'), value: { minute: '0', hour: '0', dayOfMonth: '*', month: '*', dayOfWeek: '*' } },
  { label: t('server.schedules.cronPresets.weeklySunday'), value: { minute: '0', hour: '0', dayOfMonth: '*', month: '*', dayOfWeek: '0' } },
])

function applyCronPreset(preset: { minute: string; hour: string; dayOfMonth: string; month: string; dayOfWeek: string }) {
...
async function handleDeleteSchedule() {
  ...
  try {
    await $fetch(`/api/client/servers/${serverId.value}/schedules/${schedulePendingDelete.value.id}`, {
      method: 'DELETE',
    })

    toast.add({
      title: t('server.schedules.scheduleDeleted'),
      description: t('server.schedules.scheduleDeletedDescription', { name: schedulePendingDelete.value.name }),
      color: 'success',
    })
    ...
    const message = err instanceof Error ? err.message : t('server.schedules.failedToDeleteSchedule')
    toast.add({
...
async function handleDeleteTask(taskId: string) {
  ...
  try {
    await $fetch(`/api/client/servers/${serverId.value}/schedules/${selectedSchedule.value.id}/tasks/${taskId}`, {
      method: 'DELETE',
    })
  ...
  <UPage>
    <UPageBody>
      <UContainer>
        ...
        <UCard>
          ...
          <div v-else-if="pending" class="flex flex-col items-center justify-center gap-3 py-12 text-sm text-muted-foreground">
            <UIcon name="i-lucide-loader-2" class="size-6 animate-spin text-primary" />
            <span>{{ t('common.loading') }}</span>
          </div>

          <div v-else-if="schedules.length === 0" class="flex flex-col items-center gap-3 rounded-lg border border-dashed border-default p-8 text-center text-muted-foreground">
            <UIcon name="i-lucide-calendar-clock" class="size-12 text-muted-foreground/50" />
            <div>
              <p class="text-sm font-medium text-foreground">{{ t('server.schedules.noSchedules') }}</p>
            ...
            >
              ...
              <div class="mt-4 grid gap-3 text-sm text-muted-foreground sm:grid-cols-3">
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    <p class="text-[11px] uppercase tracking-wide text-muted-foreground/70">{{ t('server.schedules.cron') }}</p>
                    <code class="rounded bg-muted px-2 py-1 text-xs text-foreground">{{ item.cron }}</code>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <UIcon name="i-lucide-list-checks" class="size-4 text-primary" />
                  <div>
                    <p class="text-[11px] uppercase tracking-wide text-muted-foreground/70">{{ t('server.schedules.tasksTitle') }}</p>
                    <p class="text-xs text-foreground">{{ t('server.schedules.taskCount', { count: item.tasks.length }) }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-2">
              ...
              <div class="mt-4 rounded-lg border border-dashed border-default/80 bg-background/50 p-3">
                ...
                <div v-else class="mt-3 space-y-2">
                  ...
                  >
                    <div>
                      <p class="font-semibold capitalize text-foreground">{{ taskActionLabel(task.action) }}</p>
                      <p class="font-mono text-[11px] text-muted-foreground">{{ getTaskDescription(task) }}</p>
                    </div>
                    <div class="text-right text-[11px] text-muted-foreground/70">
                      <p>#{{ task.sequenceId }}</p>
                      <p>{{ t('server.schedules.timeOffsetSeconds', { seconds: task.timeOffset }) }}</p>
                    </div>
                  </div>
                  <div v-if="item.tasks.length > 3" class="text-[11px] text-muted-foreground">
                    +{{ item.tasks.length - 3 }} {{ t('server.schedules.manageTasksAction').toLowerCase() }}
                  </div>
                </div>
              </div>
            </div>
    ...
    >
      <template #body>
        <div class="space-y-6">
          <div>
            <p class="text-xs font-semibold text-muted-foreground">{{ t('server.schedules.chooseTaskAction') }}</p>
          ...
          <div v-else class="space-y-2">
            <p class="text-xs font-semibold text-muted-foreground">{{ t('server.schedules.powerAction') }}</p>
            <div class="flex flex-wrap gap-2">
              <UButton
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <UFormField :label="t('server.schedules.timeOffsetLabel')" :description="t('server.schedules.timeOffsetHelp')">
              <UInput
                v-model.number="taskForm.timeOffset"
                type="number"
                min="0"
                max="3600"
              />
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-sm text-muted-foreground">{{ t('server.schedules.continueOnFailureLabel') }}</span>
            </div>
          </div>

          <div class="space-y-2">
            <div class="flex items-center justify-between">
              <p class="text-xs font-semibold text-muted-foreground">{{ t('server.schedules.currentTasks') }}</p>
              <UButton
                size="xs"
                color="primary"
            ...
            <div v-if="selectedScheduleTasks.length === 0" class="rounded border border-dashed border-default p-3 text-xs text-muted-foreground">
            ...
            <div v-else class="space-y-2">
              ...
              >
                ...
                <div class="flex flex-wrap gap-3 text-[11px] text-muted-foreground/80">
                  <span>{{ t('server.schedules.timeOffsetSeconds', { seconds: task.timeOffset }) }}</span>
                  <span v-if="task.continueOnFailure">{{ t('server.schedules.continueOnFailureLabel') }}</span>
                </div>
              </div>
            </div>
          </div>
    ...
    >
      <template #body>
        <p class="text-sm text-muted-foreground">
          {{ t('server.schedules.confirmDeleteScheduleDescription', { name: schedulePendingDelete?.name || '' }) }}
        </p>
      </template>
      <template #footer>
