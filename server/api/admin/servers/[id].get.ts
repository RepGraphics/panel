...
export default defineEventHandler(async (event) => {
  ...
  return {
    data: {
      ...
      allocationLimit: server.allocationLimit ?? limits?.allocationLimit ?? null,
      databaseLimit: server.databaseLimit ?? limits?.databaseLimit ?? null,
      backupLimit: server.backupLimit ?? limits?.backupLimit ?? 0,
