  ...
  private async executeJob(job: QueueJob) {
    ...
    switch (job.type) {
      ...
      case 'server:reinstall':
        ...
        break
      case 'backup:create':
        await this.createBackup(job.data as { serverUuid: string; name?: string; ignored?: string })
        break
      default:
...
export async function queueServerCreation(serverId: string, startOnCompletion = true): Promise<string> {
...
export async function queueBackupCreation(serverUuid: string, name?: string, ignored?: string): Promise<string> {
